...
export default function HomeScreen() {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.filesContainer} horizontal={true}>
        <Pressable style={styles.addFile} onPress={() => router.push('../components/PhotoGallery')}>
          <Image source={require('../../assets/images/create-post-camera.png')} />
          <Text style={{ fontSize: 14, marginVertical: 5 }}>Add More</Text>
          <Text style={{ color: '#7E8184' }}>{`${files.length}/10`}</Text>
        </Pressable>
      </ScrollView>
    </SafeAreaView>
  );
